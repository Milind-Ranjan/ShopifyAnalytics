...
router.get('/orders/status', async (req: AuthRequest, res) => {
  try {
    ...
    res.json(statusCounts.map((s: any) => ({ status: s.financialStatus || 'unknown', count: s._count.id })));
  } catch (error: any) {
...
router.get('/products/top', async (req: AuthRequest, res) => {
  try {
    ...
    }).sort((a: any, b: any) => b.totalRevenue - a.totalRevenue); // Sort by revenue descending
...
// Advanced Analytics Routes (DS Engine)
// Get EDA
router.get('/eda', async (req: AuthRequest, res) => {
    try {
        if (!req.tenantId) return res.status(403).json({ error: 'Tenant required' });
        const dsService = new DSService(req.tenantId);
        const data = await dsService.getEDA();
        res.json(data);
    } catch (e: any) {
...
router.get('/segmentation', async (req: AuthRequest, res) => {
    try {
        if (!req.tenantId) return res.status(403).json({ error: 'Tenant required' });
        const dsService = new DSService(req.tenantId);
        const data = await dsService.getSegmentation();
        res.json(data);
    } catch (e: any) {
...
router.get('/forecast', async (req: AuthRequest, res) => {
    try {
        if (!req.tenantId) return res.status(403).json({ error: 'Tenant required' });
        const dsService = new DSService(req.tenantId);
        const data = await dsService.getForecast();
        res.json(data);
    } catch (e: any) {
